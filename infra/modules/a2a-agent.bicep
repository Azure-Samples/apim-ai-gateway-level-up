// ------------------------------------------------------------------------------------------------
// A2A Agent Function App module
//   Provisions the .NET 8 isolated Function App that hosts the A2A "Summarizer" agent: a storage
//   account, a user-assigned managed identity, a Flex Consumption plan, and the Function App.
//   The agent serves its Agent Card at /.well-known/agent-card.json and a JSON-RPC endpoint at
//   /a2a that calls a Foundry model. It's imported into APIM as an A2A Agent API live in the
//   portal (see hol/walkthrough.md). The identity is granted "Cognitive Services OpenAI User" on
//   Foundry by the parent template so the agent can call the model keyless.
// ------------------------------------------------------------------------------------------------
...
@description('Endpoint the agent uses to call the model (Foundry account endpoint, or the APIM Foundry gateway URL).')
param foundryEndpoint string

@description('Model deployment name the agent summarizes with.')
param modelDeploymentName string

@description('Tags to apply to all resources.')
param tags object = {}

var storageAccountName = take(toLower('sta2a${uniqueSuffix}'), 24)
var appServicePlanName = '${namePrefix}-a2a-asp-${uniqueSuffix}'
var functionAppName = '${namePrefix}-a2afunc-${uniqueSuffix}'
var managedIdentityName = '${namePrefix}-a2a-id-${uniqueSuffix}'
...
resource functionApp 'Microsoft.Web/sites@2024-04-01' = {
  ...
  resource configAppSettings 'config' = {
    ...
    properties: {
      ...
      // Model access — DefaultAzureCredential uses AZURE_CLIENT_ID to select the user-assigned identity.
      FOUNDRY_ENDPOINT: foundryEndpoint
      MODEL_DEPLOYMENT: modelDeploymentName
